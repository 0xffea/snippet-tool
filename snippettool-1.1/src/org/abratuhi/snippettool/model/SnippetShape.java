...
public class SnippetShape {
	...
	public Polygon transformRectangle(Rectangle rectangle, double coefficient) {
		coefficient -= 1.0;
		Point pul = new Point((int) (rectangle.x - rectangle.width / 2 * coefficient),
				(int) (rectangle.y - rectangle.height / 2 * coefficient));
		Point pur = new Point((int) (rectangle.x + rectangle.width + rectangle.width / 2 * coefficient),
				(int) (rectangle.y - rectangle.height / 2 * coefficient));
		Point plr = new Point((int) (rectangle.x + rectangle.width + rectangle.width / 2 * coefficient),
				(int) (rectangle.y + rectangle.height + rectangle.height / 2 * coefficient));
		Point pll = new Point((int) (rectangle.x - rectangle.width / 2 * coefficient), (int) (rectangle.y
				+ rectangle.height + rectangle.height / 2 * coefficient));
	...
	public static Polygon rotatePolygonO(Polygon oldpoly, double d, Point c) {
		...
		for (int i = 0; i < oldpoly.npoints; i++) {
			newpoly.xpoints[i] = (int) (+(oldpoly.xpoints[i] - c.x) * Math.cos(d) + (oldpoly.ypoints[i] - c.y)
					* Math.sin(d) + c.x);
			newpoly.ypoints[i] = (int) (-(oldpoly.xpoints[i] - c.x) * Math.sin(d) + (oldpoly.ypoints[i] - c.y)
					* Math.cos(d) + c.y);
		}
		return newpoly;
	}
