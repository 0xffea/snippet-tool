	...
	public static File[] cutSnippets(File inputImageFile, ArrayList<InscriptCharacter> characters, String directory, String basename){
		...
		for(int i=0; i<characters.size(); i++){
			...
			BufferedImage outputImage = inputImage.getSubimage((int)Math.max(0,characterSnippet.getX()), 
					...
					(int)Math.min(inputImage.getWidth()-characterSnippet.getX(), characterSnippet.getWidth()), 
					(int)Math.min(inputImage.getHeight()-characterSnippet.getY(), characterSnippet.getHeight()));
			outputImageFiles[i] = new File(directory + basename + "_" + characters.get(i).inscript+"_"+characters.get(i).getNumber()+".png");
